...
def image_to_tfexample(image_data, image_id, image_format, height, width, class_id):
...
def _convert_dataset_daan(split_name, filenames, class_names_to_ids, trainval_id_dict, dataset_dir, tfrecord_filename, _NUM_SHARDS):
  ...
  Args:
    ...
    class_names_to_ids: A dictionary from class names (strings) to ids
      (integers).
      image_id: to identify output of Model
    dataset_dir: The directory where the converted datasets are stored.
  """
  assert split_name in ['train', 'traintest', 'validation']
      ...
      for shard_id in range(_NUM_SHARDS):
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            id_of_image = os.path.basename(filenames[i])
            identifier_image = trainval_id_dict[id_of_image]
            
            
            example = image_to_tfexample(
                image_data, identifier_image, 'png'.encode(), height, width, class_id)
...
def _dataset_exists(dataset_dir, _NUM_SHARDS, output_filename):
  for split_name in ['train', 'traintest', 'validation']:
    for shard_id in range(_NUM_SHARDS):
